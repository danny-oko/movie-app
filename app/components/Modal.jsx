"use client";
import Link from "next/link";

const Modal = ({ genres = [], onSelect }) => {
  return (
    <div className="absolute left-0 top-12 z-50 w-[572px] h-[332px] rounded-2xl border border-border bg-popover shadow-xl">
      <div className="p-6">
        <h3 className="text-3xl font-semibold text-foreground">Genres</h3>
        <p className="mt-1 text-lg text-muted-foreground">
          See lists of movies by genre
        </p>

        <div className="my-5 h-px w-full bg-border" />

        <div className="flex flex-wrap gap-3 max-h-[190px] overflow-auto pr-1">
          {genres.map((g) => (
            <Link
              href={`/pages/genres/${g.id}`}
              key={g.id}
              type="button"
              onClick={() => onSelect(g.name)}
              className="rounded-full border border-border bg-background px-4 py-2 text-sm font-medium text-foreground hover:bg-accent"
            >
              {g.name}
            </Link>
          ))}
        </div>
      </div>
    </div>
  );
};

export default Modal;
